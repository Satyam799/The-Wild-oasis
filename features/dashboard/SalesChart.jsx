...
const data=allDay.map((date)=>{
  return {
    ...
    totalSales: bookings.filter((el)=>isSameDay(date,new Date(el.created_at))).reduce((acc,crr)=>acc+crr.totalPrice,0), 
    extrasSales: bookings.filter((el)=>isSameDay(date,new Date(el.created_at))).reduce((acc,crr)=>acc+crr.extrasPrice,0)
  }
})

console.log(data)

const {Darkmode:isDarkMode}=Uppost()
...
const colors = isDarkMode
  ...
  return (
    <StyledSalesChart>
        <Heading as='h2'>Sales from {format(allDay.at(0),'MMM dd yyyy')} &mdash; {format(allDay.at(-1),'MMM dd yyyy')}</Heading>
